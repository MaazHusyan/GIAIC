...
let dataFetch = async (data: string) => {
  try {
    let apiFetch: any = await fetch(data);
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (ans.quiz == data[i].correct_answer) {
        ...
        console.log(
          `${chalk.red.italic(
            `\tCorrect Answer is ${`${chalk.redBright.italic(
    ...
    console.log(
      `\t${chalk.blue(
        `${user.userName} Your Score is: `
      )} ${chalk.blueBright.italic(playerScore)}`
    );
  } catch (error) {
